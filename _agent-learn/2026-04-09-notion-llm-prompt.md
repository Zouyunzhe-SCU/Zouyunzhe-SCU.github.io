...
我为你制定了一份**“保姆级”实操方案。这是一套**完全基于你现有 P100 硬件条件**，且能够发表在 **QIMS (2区)** 或同级别期刊上的完整路线图。
...
2. **问题：** 现有方法（如 SWITCH 2025）虽然利用了多数据集，但缺乏医学文本知识的指导，且对算力要求高。
3. **方法：** 我们提出了 **Prompt-TBI**。
    - (1) 利用 **LLM** 生成各出血亚型的解剖学描述，构建语义提示 (Text Prompt)；
    - (2) 设计了基于 **MedSAM** 的参数高效微调架构，在 P100 上实现了多数据集联合训练；
...
1. **今晚的任务：** 去下载 **MedSAM** 的代码（GitHub 上搜 `bowang-lab/MedSAM`），先跑通它的 Demo。
2. **明天的任务：** 写个 Python 脚本，调用 CLIP 或 BERT，把你用 ChatGPT 生成的那 5 句描述变成向量（Embedding），看看长什么样。
3. **心态建设：** 你现在的思路（LLM + MedSAM + 多数据集）是 **2025 年最主流、最容易中** 的方向。那篇 SWITCH 的文章证明了多数据集可行，那篇 QIMS 的文章证明了弱监督可行，你把它们结合起来，加个 LLM，**逻辑闭环完美**。

# 2025-12-10

## cvpr的比较新的

### Enhancing SAM with Efficient Prompting and Preference Optimization for
Semi-supervised Medical Image Segmentatio
...
![image.png](image/2026-04-09-notion-llm-prompt/image.png)
...
![image.png](image/2026-04-09-notion-llm-prompt/image 1.png)

### **LViT: Language Meets Vision Transformer in Medical Image Segmentation**

https://ieeexplore.ieee.org/document/10172039

![image.png](image/2026-04-09-notion-llm-prompt/image 2.png)
...
### 第三部分：这篇综述如何具体指导你的毕设？（实战映射）

这是最重要的部分。我根据综述里的结论，结合你手里的 **BCIHM (强标)**、**BHSD (弱标)** 和 **MBH_Val (多专家)** 数据，为你制定了毕设的**“三步走”**策略：
...
- **你的操作：**
    ...
    - **论文话术：** “根据 Zhang 等人的综述建议，直接应用 SAM 存在域偏移问题。因此，本研究采用 Parameter-Efficient Fine-Tuning (PEFT) 策略，在 SAM 编码器中嵌入适配器，利用 BCIHM 等数据集进行特定域的特征对齐。”

### 步骤二：引用“自动提示”，利用弱标签数据

- **综述说了什么：** 综述特别提到，**"Box prompts outperform point prompts"（框提示优于点提示）**，且可以通过分类网络的热力图（CAM）自动生成框。
- **你的操作：**
    - **数据：** 用你的 **BHSD (1980例)**，这部分数据只有 `anybleed/nobleed` 标签。
    - **方法：** 训练一个简单的分类网络（如 ResNet-18）去分有没有血。提取它的 **CAM (类激活图)**，把热力图高亮的地方画一个矩形框。把这个**矩形框**作为 Prompt 喂给 SAM。
    - **结果：** 这样你就“白嫖”了 1980 例数据的分割结果（伪标签），可以反过来提升你的主模型。
    - **论文话术：** “针对 TBI 数据标注稀缺的问题，受综述中‘基于检测的提示生成’启发，本研究设计了弱监督提示生成模块，从 BHSD 分类数据中挖掘空间定位信息。”

### 步骤三：引用“多模态与不确定性”，拔高多专家数据

- **综述说了什么：** 综述的“未来展望（Future Directions）”部分通常会提到：**“Handling Ambiguity（处理歧义性）”** 是 SAM 这种能输出多个 Mask 的模型的天然优势。SAM 默认会输出 3 个不同层级（Whole, Part, Sub-part）的掩码。
- **你的操作：**
    - **数据：** 用你的 **MBH_Val (48例多专家)**。
    - **方法：** 强行利用 SAM 输出的 3 个 Mask。对比这 3 个 Mask 和 3 个医生标注的差异。计算**IoU**的同时，计算**不确定性分布**。
    - **论文话术：** “综述指出，医学图像中病灶边界往往具有主观性。本研究利用 SAM 的多模态输出特性（Multi-mask Output），结合多专家标注数据，首次在 TBI 任务中实现了基于偏好对齐的歧义性建模。”

### 总结

这篇综述对你的意义在于：

1. **它证明了你“不直接用 U-Net 而用 SAM”是合理的**（紧跟技术潮流）。
2. **它提供了“微调+提示工程”的方法论支撑**（帮你的技术路线背书）。
3. **它指出了“解决模糊性”是未来方向**（让你用多专家数据做偏好优化显得非常有远见）。